...
export const BarChart = props => {
  ...
  const scaleX = D3.scaleLinear()
    .domain([0, D3.max(planetOrdinals) + 1])
    .range([0, 270]);
  ...
  const xAxis = D3.axisBottom(scaleX)
    .tickSize(8)
    .tickValues([...planetOrdinals])
    .tickFormat(d => d);
  const yAxis = D3.axisLeft(scaleAxisY)
    .tickSize(4)
    .tickValues([100, 500, 1000, 1500, 2000])
    .tickFormat(d => d);
  ...
  svg.append('g').attr('class', 'axis').attr('transform', `translate(36, 40)`).call(yAxis);
