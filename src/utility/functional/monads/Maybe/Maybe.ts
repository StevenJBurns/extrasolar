...
const map = <T, U>(fn: (t: T) => U) => (maybe: Maybe<T>): Maybe<U> =>
  maybe.type === JUST ? Just(fn(maybe.value)) : Nothing();

const chain = <T, U>(fn: (t: T) => Maybe<U>) => (maybe: Maybe<T>): Maybe<U> =>
  maybe.type === JUST ? fn(maybe.value) : Nothing();

const fold = <T, U>(onNothing: () => U, onJust: (t: T) => U) => (maybe: Maybe<T>): U =>
