// src/utilities/functional/core/curry/curry.test.ts
...
describe('curry functional utility', () => {
  it('correctly curries a function with two arguments', () => {
    ...
    expect(curriedAdd(1)(2)).toBe(3); // TS might still need help
    const addOne = curriedAdd(1) as (b: number) => number; // Assert if needed
  ...
  it('correctly curries a function with three arguments', () => {
    ...
    const mulTwo = curriedMultiply(2) as (b: number) => (c: number) => number; // Assert
  ...
  it('handles partially applied functions', () => {
    ...
    const subtractFrom10 = curriedSubtract(10) as (b: number) => number; // Assert
  ...
  it('correctly handles curried functions with multiple argument sets', () => {
    ...
    const div100 = curriedDivide(100) as (b: number) => (c: number) => number; // Assert
  ...
  it('works with functions of different arities', () => {
    ...
    const hello = curriedGreet('Hello') as (name: string) => string; // Assert
